...
public class TcTable4 extends Baseclass
{	
	LoginPage lp;
	HomePage hp;
	EmpMgtPage emp;
	...
	public void PrintEmpWithSubUnitQATEAM()
	{
		...
		String expJobTitle = "QA Engineer";	
		int IndiaOfficeColumnIndex = TData.get(0).indexOf(expColumn);	
		for(int i=1; i<TData.size();i++)
		{	
			if(expOfficeLocationValue.equals(TData.get(i).get(6)) && expJobTitle.equals(TData.get(i).get(2)) )
			{
